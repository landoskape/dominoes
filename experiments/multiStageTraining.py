...
def trainValueAgent(numPlayers, highestDominoe, shuffleAgents, trainGames, trainRounds):
    ...
    game = dg.dominoeGame(highestDominoe, numPlayers=numPlayers, shuffleAgents=shuffleAgents, agents=agents, defaultAgent=da.persistentLineAgent, device=device)
...
def plotResults(results):
    ...
    ax[1].plot(xs1, filter(results['s1HandWinnerCount'][:,0]/trainRounds), c='b', label=f"{args.value_agent}")
    ax[1].plot(xs2, filter(results['s2HandWinnerCount'][:,0]/trainRounds), c='b')
    ax[1].plot(xs1, filter(np.mean(results['s1HandWinnerCount'][:,1:],axis=1)/trainRounds), c='k', label='dominoeAgent')
    ax[1].plot(xs2, filter(np.mean(results['s2HandWinnerCount'][:,1:],axis=1)/trainRounds), c='r', label='persistentLineAgent')
    ax[1].set_ylim(0)
    ax[1].set_xlabel('Training Games')
    ax[1].set_ylabel('Training # Won Hands')
