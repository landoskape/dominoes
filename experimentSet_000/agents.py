...
class valueAgent(dominoeAgent):
    ...
    def saveAgentParameters(self, path, description=None):
        ...
        if not(description): description = f"Saved network parameters from {self.agentName}."
        parameters = np.array([agentParameters, networkParameters, description])
        ...
        savePath = path / modelName
        np.save(path / modelName, parameters) 
        return str(savePath)+'.npy'
